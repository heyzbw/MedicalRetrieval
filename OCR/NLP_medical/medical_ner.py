...
import json
from NLP_medical.utils import load_vocab
from NLP_medical.ner_constant import *

from NLP_medical.model_ner import BERT_LSTM_CRF
import os


class medical_ner(object):
    def __init__(self):
        self.NEWPATH = './NLP_medical/model_ner/medical_ner/model.pkl'
        self.vocab = load_vocab('./NLP_medical/model_ner/medical_ner/vocab.txt')
        self.vocab_reverse = {v: k for k, v in self.vocab.items()}

        self.model = BERT_LSTM_CRF('./NLP_medical/model_ner/medical_ner', tagset_size, 768, 200, 2,
        ...
        for i, dev_batch in enumerate(test_loader):
            ...
            if entity_mark is not None:
                for item, ent in entity_mark.items():
                    ...
                    if (tag_dic[tag] in entity_list):
                        entity_list[tag_dic[tag]].append(entity)
                    else:
