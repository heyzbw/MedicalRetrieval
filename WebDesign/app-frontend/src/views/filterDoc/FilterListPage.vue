<template>
    <div class="doc-group">
        <div class="doc-thumb-group">
            <DocThumb class="doc-thumb" v-for="item in data" :flag="false" :title="item.name" :docId="item.thumbId"
                @click.native="getDocView(item.id)"></DocThumb>
        </div>
        <div class="doc-group-page">
            <Page :page-size="pageSize" :current="pageNum" :total="total" @on-change="pageChange" />
        </div>
    </div>
</template>
